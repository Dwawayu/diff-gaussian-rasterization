	...
	float* __restrict__ dL_dzs)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// gradients w.r.t. opa (blending factor for a Gaussian/pixel
			// pair).
			float dL_dopa = 0.0f;
...
// Backward version of the rendering procedure.
template <uint32_t C, uint32_t CHANNEL_MISC>
__global__ void __launch_bounds__(BLOCK_X * BLOCK_Y)
renderCUDA(
	const uint2* __restrict__ ranges,
	const uint32_t* __restrict__ point_list,
	...
	float* __restrict__ dL_dzs)
{
	...
	// 	dL_dmisc[ch] = 0;
	// }
	float accum_misc_rec[CHANNEL_MISC] = { 0 };
	float last_misc[CHANNEL_MISC] = { 0 };
	float dL_dmisc[CHANNEL_MISC] = { 0 };
	...
	float* dL_dz)
{
	if (channel_misc == 0){
	...
	else if (channel_misc < 4){
		renderCUDA<NUM_CHANNELS, 3> << <grid, block, channel_misc * BLOCK_SIZE * sizeof(float)>> >(
			ranges,
			point_list,
			W, H,
			bg_color,
	...
	else if (channel_misc < 7){
		renderCUDA<NUM_CHANNELS, 6> << <grid, block, channel_misc * BLOCK_SIZE * sizeof(float)>> >(
			ranges,
			point_list,
			W, H,
			bg_color,
	...
	else if (channel_misc < 10){
		renderCUDA<NUM_CHANNELS, 9> << <grid, block, channel_misc * BLOCK_SIZE * sizeof(float)>> >(
			ranges,
			point_list,
			W, H,
			bg_color,
		...
		printf("Error: size of misc is not supported.\n");
	}
